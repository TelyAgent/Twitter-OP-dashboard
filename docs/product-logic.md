...
> 4 个页面按数据加工深度形成流水线：监控源 → 热点雷达 → 模板库 → 每周复盘。
>
> **索引**：[`docs/README.md`](README.md) — 完整文档索引和阅读路径

## 概述

OP-dashboard 是一个 **Web3 内容运营工业化流水线**，覆盖从监控源管理、热点发现、模板提炼到内容生成和产品复盘的全链路。系统采用本地优先架构，4 个静态 HTML 页面 + Node.js 静态服务，Supabase 远程持久化，DeepSeek API 云端 AI 推理，OpenCLI Chrome 插件抓取 Twitter 数据。
...
> 架构细节参见 [`ARCHITECTURE.md`](../ARCHITECTURE.md)。功能清单参见 [`docs/SPEC.md`](SPEC.md)。
...
> **Mock**：当前实现为 `src/api/ai-recommendations.json` 静态文件，4 个候选人硬编码。真实的 AI 推荐逻辑待实现。
...
默认模式下，提炼数据源为 `VIRAL_POOL`（8 条硬编码假推文），非真实 DB 数据。切换到"粘贴推文链接"模式则走真实 `Provider.fetchSingleTweet()` 路径。

```
爆款推文 (VIRAL_POOL 假数据 或 粘贴真实 URL)
...
- **爆款推文池**：按视角分类，含提炼按钮（浏览区数据来自 DB，提炼源为 VIRAL_POOL 假数据）
...
- [`docs/SPEC.md`](SPEC.md) — 每页功能清单
- [`docs/TEST_PLAN.md`](TEST_PLAN.md) — 如何验证各环节
- [`../ARCHITECTURE.md`](../ARCHITECTURE.md) — 技术架构和代码位置
